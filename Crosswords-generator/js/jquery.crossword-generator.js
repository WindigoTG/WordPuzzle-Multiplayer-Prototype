(function($) {
  ...
  Word.prototype = {
    ...
    Clue.prototype = {
        ...
        // Clones itself
        clone: function () {
            var clue = new Clue(this.number, this.value);
    ...
    clone: function() {
      ...
      for (var i = 0; i < this.words.length; ++i) {
        ...
        for (var i = 0; i < this.clues.length; ++i) {
            crossword.clues.push(this.clues[i].clone());
        }
  ...
  CrosswordGenerator.prototype = {
    ...
    init: function(words, clues) {
      ...
      var state = new GeneratorState(words, clues);
  ...
  function GeneratorState(words, clues) {
      this.crossword = new Crossword();
      
      for (var i = 0; i < clues.length; i++) {
          this.crossword.clues.push(clues[i]);
      }
    ...
    var init = function(options) {
      ...
      for (var i = 0; i < opts.words.length; ++i) {
        ...
        for (var i = 0; i < opts.clues.length; ++i) {
            clues.push(new Clue(i + 1, opts.clues[i]));
        }
      
      theGenerator = new CrosswordGenerator(opts.debug);
      theGenerator.init(words, clues);
    };
