(function($) {
  ...
  $(document).ready(function() {
    ...
    $('button[name=savejson]').click(savejsonClickHandler);
  ...
  function generateClickHandler(/*event*/) {
    ...
    $('ul.inputData.inputWords li').each(function(index, element) {
      ...
      $('ul.inputData.inputClues li').each(function (index, element) {
          //var clue = $.trim($(element).find('input[name=clue]').val());
    ...
    if (! words.length) {
      ...
      $.crosswordGenerator('init', { words: words, clues: clues, debug: false });
    
    if (generateNextCrossword()) { // Generate first alternative
      // Select and display first alternative
      currentCrosswordIndex = 0;
	  console.log(crosswords4UI[currentCrosswordIndex].crossword);
  ...
  // Handler for "Savejson" clicks
  function savejsonClickHandler(/*event*/) {
    var a = document.createElement("a");
	var jsonData = JSON.stringify(crosswords4UI[currentCrosswordIndex].crossword);
